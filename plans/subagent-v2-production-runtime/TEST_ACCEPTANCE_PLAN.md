...
本节冻结 C7c 的目标验收 Oracle。C7c-1、Worker/Process 离线 case、C7c-4 HTTP wire-security case、C7c-5b poll/route/receipt case、C7c-5c create/replay Store case 与 C7c-5d-a current-attachment delivery/ACK/wait 的 STORE04/STORE07 已有对应实现和 manifest 映射；Core external reconnect prerequisite 已有额外内部对抗回归，但不计作 HTTP placement evidence。矩阵中尚未覆盖的 Worker/Process L5/L6/Ark、HTTP production durable Store/runtime/delivery response/attachment recovery/durable reconnect/L5/L6/Ark 条目仍是 `pending_external_evidence` 或未满足 requirement，不能据此宣称 `C7-WORKER`、`C7-PROCESS`、`C7-HTTP`、`C7-AUTH` 或 Phase 2 passed。C7c 仍只改变 child execution placement，父 Agent、root run、owner session 和审批控制权都留在 controller，不实现 handoff。
