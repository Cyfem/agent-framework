...
- 当前实现状态：当前 checkout 已完成 C0～C6 的 Core v2、官方 Local、公开 Agent durable loop、Phase 1 离线/进程恢复证据，以及 C7a/C7b/C7c-1 的 Core transport/RPC/control/Peer/artifact-sidecar、target registry、controller/target bridge、controller-owned Model gateway、C7c-2/C7c-3 离线 Worker/Process placement、C7c-4 HTTP wire-security 基础和仅同进程 resident task 生效的 Core external reconnect 前置。`C7-WORKER`、`C7-PROCESS`、`C7-HTTP` 与 `C7-AUTH` requirement 因缺少 L5/L6/Ark 或完整 placement 证据仍为 `planned`；HTTP Executor/job/durable reconnect 尚未交付，Phase 2 未通过，C8/C9 与 Docker/Linux/真实方舟最终证据仍是待交付要求。
