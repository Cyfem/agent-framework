...
- C7 Core 已有 closed transport、strict RPC/control、双向 Peer、replay/rollover 与 artifact sidecar 的 L1 测试和 manifest 映射；Worker 与 Process 包及其离线 acceptance 已交付，HTTP 包的 multipart/HMAC/replay/authz L1/L2 安全基础也已交付。Worker/Process 仍缺 L5 真实方舟 placement 与 L6/Docker/Linux live gate，HTTP 仍缺 Executor/job/listener/reconnect、L5/L6，不能把 manifest 的 `C7-WORKER`、`C7-PROCESS`、`C7-HTTP` 或 `C7-AUTH` requirement 改为 implemented。C8/C9 的 PostgreSQL、BullMQ、Docker、S3、OTel、Compose 与 release report 同样不能由这些离线证据代替。
...
本节冻结 C7c 的目标验收 Oracle。C7c-1、Worker/Process 离线 case 与 C7c-4 HTTP wire-security case 已有对应实现和 manifest 映射；矩阵中尚未覆盖的 Worker/Process L5/L6/Ark、HTTP job/runtime/L5/L6/Ark 条目仍是 `pending_external_evidence` 或未满足 requirement，不能据此宣称 `C7-WORKER`、`C7-PROCESS`、`C7-HTTP`、`C7-AUTH` 或 Phase 2 passed。C7c 仍只改变 child execution placement，父 Agent、root run、owner session 和审批控制权都留在 controller，不实现 handoff。
