...
- 设计状态：总体方向与 Phase 1 Oracle 已冻结；当前 checkout 已完成 C0～C6、C7a/C7b、C7c-1 的 Core transport/RPC/control/Peer/artifact-sidecar、target registry、controller/target bridge 与 controller-owned Model gateway、C7c-2/C7c-3 的离线 Worker/Process placement、C7c-4 的 HTTP signed multipart/HMAC/replay/authz 安全基础、C7c-5a 的仅同进程 resident task external reconnect 前置、C7c-5b 的 closed poll/route/receipt wire 基础、C7c-5c 的原子 create/replay Job Store 地基、C7c-5d-a 的 Store IO context 与 current-attachment delivery/ACK/wait Memory ledger，以及 C7c-5d-b 的 closed poll delivery response wire；HTTP Executor、production durable Store、response authentication/真实网络交付、attachment recovery、listener/heartbeat/durable reconnect 仍待 C7c-5 后续批次。
...
当前实现边界以源码与包 README 为准：Core v2、官方 Local、Memory/Atomic File StateStore、公开 Agent durable loop、跨协议/跨进程 Phase 1 恢复和离线验收，以及 C7 Core transport/RPC/control/Peer/artifact-sidecar、target registry、controller/target bridge、controller-owned Model gateway、同进程 external reconnect 前置、C7c-2/C7c-3 离线 Worker/Process placement、C7c-4 HTTP wire-security、C7c-5b poll/route/receipt wire、C7c-5c create/replay Store、C7c-5d-a current-attachment delivery Store 与 C7c-5d-b closed poll delivery response wire 已经落地；`C7-WORKER`、`C7-PROCESS`、`C7-HTTP` 与 `C7-AUTH` requirement 因缺少 L5/L6/Ark 或完整 HTTP placement 证据仍为 `planned`，HTTP production durable Store/runtime/response authentication/真实网络交付/attachment recovery 与 Phase 3 适配器仍按后续章节实施，Phase 2 尚未通过。真实方舟与 Docker/Linux live gate 未执行时不得标记为通过。
...
target bridge 默认仍只允许 settled resident 的只读 reconnect。只有显式 `reconnect: 'external_binding'`、以 `(channelId) => Peer` 的 channel-aware provider 精确解析 trusted attachment，且同时提供 `checkpointCommitted(context)` 时，才开放 process-local live replacement；该模式传 scalar Peer 会在构造阶段 fail fast。hook 是 trusted placement SPI，必须按 owner/task、attempt/epoch/fencing 与 operationId 做 durable、幂等 CAS，并在 signal/deadline 失效后拒绝 late write。每个 checkpoint-bearing control 从 Peer 同步 admission 到 controller reply、hook 和本地 checkpoint 更新全链路串行；method/scope mismatch、reply-loss、timeout 或 hook failure 都留下 gap，不能回退到更旧 checkpoint，admission 前即可证明的 closed-channel 失败则保留既有 durable checkpoint。reconnect 只能来自与当前 attachment 不同的 trusted `channelId`，先 soft-quiesce 并排空已准入的 mutating control/checkpoint 与 provider Model exchange，再 hard-fence/abort 旧 runner；已准入 events 可按原 attachment 完成，但 hard fence 后的迟到结果会被 quarantine。provider Model exchange 按 `providerOperationId` single-flight；target 在 Peer 前重算 canonical request hash，transport admission 后未见完整 correlated reply 时只允许同 ID/hash 查询原 durable result，权威 `outcomeUnknown`、不同语义或 runner 带未决结果 settle 都把 unknown 单调终结，禁止 resume/reconnect/第二次 provider dispatch。旧 channel 与旧 generation 的 cancel/snapshot/events/Model/control/settlement 都固定拒绝。合法 fresh runner 只从最后 controller-ACK 且 placement committed 的兼容 checkpoint 启动，factory/deadline 失败保留 detached slot 供后续更高 scope + 另一 fresh channel 重试，不 create 替代 task。该 API 不导出/恢复 bridge task map、Peer pending exchange 或 sequence，也不能跨 bridge/process restart；完整 HTTP durable reconnect 仍须额外 job/store/spool/attachment generation 和 controller restore 状态机。
...
这些离线证据不包含真实方舟 L5、分布式/部署级 L6 或 Docker/Linux live gate；因此 manifest 中 `C7-WORKER` requirement 继续保持 `planned`。C7c-3 的 Process 离线实现也复用了相同 F08/F09/F19 Oracle，并增加 advanced IPC、环境/stdio、send callback/backpressure、sendHandle、cancel/kill、exit/close 与 controller 断连孤儿 watchdog；`C7-PROCESS` 同样因缺少 L5/L6/Ark 保持 `planned`。Worker 与 Process 都只执行宿主信任的 target，不是网络、文件系统或操作系统安全沙箱；Process 只承诺回收受控直属 child，不承诺任意进程树。HTTP wire、create/replay Store、current-attachment delivery Store 与 closed poll delivery response 地基已经交付；HTTP Executor、response authentication/真实网络交付、production durable Store、attachment recovery、listener 与 durable placement 仍待实现，Phase 2 未通过。
...
- `jobs.poll` 的 transport-command 分支使用精确 `application/vnd.maneeagent.poll+json` 与最大 4 KiB 的 closed `{version:'1',channelId,channelGeneration,ackCursor,waitMs}`；generation/cursor 是 canonical decimal uint64，wait 为 `0..10000`。该 codec 不查询 job、不确认 delivery、不启动 long poll。remote delivery cursor、Core event sequence 与 Peer envelope sequence 保持三个独立域。
- closed route policy 固定映射：heartbeat 不接收 packet；create/resume/reconnect 只接收 operation/mode 精确匹配的 `executor.request`；cancel 只接收 `cancel.request`；poll multipart 分支只接收 `snapshot.request|events.request`；control-reply 只接收 `control.reply|model.reply|events.page|protocol.error` 且 path requestId 等于 correlationId。`createHttpSubAgentPacketSemanticReceipt()` 对已严格验证的 routed packet 计算 JCS SHA-256，保留 route kind/job scope、RPC kind/task/operation/payload 与排序 sidecar descriptor，排除 HMAC、boundary、Peer identity/correlation 和 control path requestId；它不代替 job idempotency/delivery ledger。
- C7c-5c 的 `createHttpSubAgentJobCreateIdentity()` 只接受 strict `jobs.create + executor.request/create`，以 `{version,mode,requestWithoutRemainingMs,sidecarDescriptorsSortedBySidecarId}` 的 JCS SHA-256 生成 create identity，同时保留首次完整 semantic receipt 和 owned packet。它排除会衰减的 `remainingMs` 与 HTTP/Peer delivery identity，但业务 request、operation/idempotency、task scope 与 sidecar digest 仍全部参与 identity。
- `HttpSubAgentJobStore.createOrReplay()` 必须在 `(principalId,ownerSessionId,idempotencyKey)`、`(principalId,jobId)` 和 `(principalId,ownerSessionId,taskId)` 三条索引上原子 linearize。首写 record 固定 `state='created'`、`revision='0'`、`channelGeneration='0'`；首次 `remainingMs` 成为不可扩张 ceiling。相同 create identity 的更小/相等 deadline replay 原 job，且不刷新 packet/timestamp/revision；identity、job、task 或 deadline 扩张冲突都 fail closed。
- `load()` 需要完整 principal/owner/job scope；`resolveForAuthorization()` 只按 authenticated principal/job 返回 frozen owner scope，供随后独立 authz，授权前不得复制或暴露 full packet。`MemoryHttpSubAgentJobStore` 仅允许显式 `loopback-test`，同时按 job 数与 canonical frame UTF-8 + canonical sidecar descriptor JSON UTF-8 + raw sidecar bytes 有界，不 LRU/TTL/逐 job 删除；dispose 归零 Store-owned sidecar bytes并清空索引。
- C7c-5d-a 的 `HttpSubAgentJobDeliveryStore` 只在 initial `channelGeneration='0'` 上提供 `loadDelivery/enqueueOutbound/pollOutbound/waitForOutbound` semantic mutation。Store IO context 只含 caller-local `signal/deadlineAt`，不持久化、不扩张 job deadline，dispose 不可取消。outbound normalize 必须 strict decode/canonical re-encode完整 Core envelope并验证 sidecar digest；packet receipt 保留 channel/sequence/message/correlation，加排序 descriptor，不能复用会排除 transport identity 的 routed receipt。
- Memory delivery ledger 同时按 messageId 与独立 target→controller Peer sequence保留 compact receipt；首包 sequence=`1`、之后连续，remote cursor另行从 `1` 连续分配。每次 poll 最多 offer 一包；current ACK 重放 offer，只有 exact offered cursor 可原子确认并返回下一包，stale/skip/future冲突。ACK 从 delivery bytes账本释放 canonical frame/descriptor/raw bytes、归零 mutable sidecar并保留 compact receipt；wait 在同一临界区 recheck revision+注册，timeout/abort/deadline/dispose一次性清 timer/listener/capacity。
- C7c-5d-b 的 delivery response codec 始终编码 `200 + Cache-Control: no-store + multipart/mixed`，empty 也保留同一 closed JSON first part。`requestReceipt` 从 canonical `jobs.poll` route与完整 poll command内部重算，只在已验证 TLS endpoint 上做 semantic correlation，不是 response MAC。decode 精确重验 initial generation `0`、channel/ACK、`cursor=ack+1`、inner Core channel、target→controller kind、完整 transport packet receipt 与 sidecar；remote cursor 不与 Peer sequence 比较。
- 5d-a/5d-b 不开放 attachment/generation rotate：旧 raw Core packet不能跨 fresh Peer channel接收，当前 Peer又不导出 pending/sequence。后续 recovery mutation必须同时具备 authoritative checkpoint/terminal/provider proof 与 Peer rebase/import（或等价 semantic reconstruction），不能以 force flag、裸 CAS 或静默 purge绕过。本批也没有 response authentication/真实网络交付、listener/client、production durable adapter或完整 job lifecycle。
...
验证顺序固定为：closed headers/route/body bounds → raw body digest → timestamp/key/encoding → signature constant-time compare → 原子 `ReplayCache.consume(keyId, nonce, expiresAt)`（TTL 固定 120 秒）→ 无业务副作用地完整解码 multipart/closed packet → 由 route、packet 或 C7c-5 的受信 job state 解析 owner session → `authorize(authContext, { ownerSessionId, method, routeId })` → closed route↔RPC policy → Peer/Core。authorization scope 的 `method` 固定为 canonical HTTP `POST`，endpoint 权限由 `routeId` 表示；不能把未经 decode 的 frame 字符串当权限字段。owner session 位于签名 body 或受信 job state，不能通过新增未签名 header 绕过；heartbeat 是唯一允许 `ownerSessionId = null` 的 endpoint-scope route，其余 route 缺失或非法 owner scope 固定为认证后的 400。生产 handler 要求 distributed replay cache，内存实现只允许显式 loopback/test。未知 key、错签名、过期、replay、path/query 不规范和未授权对外都返回同一安全 401；认证成功后的 multipart/RPC/scope malformed 返回固定 400，key/replay/authz backend 不可用返回固定 503，Peer/Core callback 异常返回固定 500。所有错误都必须脱敏；授权拒绝时进入 Peer/Core/runner 的 callback 次数为 0，session ID 本身不构成授权。
...
- `packages/executor-http`（wire-security、poll/route/receipt、create/delivery Store 与 closed response foundations 已交付；待发布 `2.0.0`，完整 Executor/L5/L6/Ark 未完成）
