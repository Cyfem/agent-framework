...
- 设计状态：总体方向与 Phase 1 Oracle 已冻结；当前 checkout 已完成 C0～C6、C7a/C7b、C7c-1 的 Core transport/RPC/control/Peer/artifact-sidecar、target registry、controller/target bridge 与 controller-owned Model gateway、C7c-2/C7c-3 的离线 Worker/Process placement、C7c-4 的 HTTP signed multipart/HMAC/replay/authz 安全基础、C7c-5a 的仅同进程 resident task external reconnect 前置、C7c-5b 的 closed poll/route/receipt wire 基础、C7c-5c 的原子 create/replay Job Store 地基，以及 C7c-5d-a 的 Store IO context 与 current-attachment delivery/ACK/wait Memory ledger；HTTP Executor、production durable Store、delivery response、attachment recovery、listener/heartbeat/durable reconnect 仍待 C7c-5 后续批次。
