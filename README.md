...
- **Subagent v2**：通过 typed `SubAgentDefinition`、模型可见 `{ subAgent, executor, input }` wire 和独立 Executor 调度隔离 child Agent；支持跨协议 placement、审批暂停/恢复、嵌套委派、持久 checkpoint、取消与 typed result，并提供 C7 transport v1、strict RPC/control、双向 Peer、artifact sidecar 与可恢复 Executor settle contract。
...
- 当前 checkout 已实现宿主进程内 Local Executor、单机 Memory/Atomic File StateStore，以及 C7 Core 共用的 closed JSON transport v1、12-kind strict RPC、16-method control dispatcher/proxy、双向 Peer、32 MiB artifact sidecar 和 `recovery_required` settle contract。Remote control proxy 暂不提供 artifact `put`；Worker、Process、HTTP 具体 placement 与分布式生产适配器仍属于后续 C7–C9，Phase 2 尚未通过，且当前 Local 包尚未发布到 npm。
