...
const TARGET_ENTRY = new URL('./fixtures/worker-f19-target.mjs', import.meta.url);
const DEFINITION_REF = Object.freeze({ name: 'worker-f19-child', version: '2' });
const PROOF = 'worker-f19-provider-proof';
const manifestDefinition: SubAgentDefinitionRegistration = Object.freeze({
  ...DEFINITION_REF,
  description: 'Manifest-only Worker F19 definition.',
  inputSchema: z.json(),
  outputSchema: z.json(),
});
...
function createHarness(
  ...
  const crashChannel = new BroadcastChannel(`maneeagent-worker-f19-worker-f19-${mode}-${suffix}`);
  let crashChannelClosed = false;
  const sdkCreate = vi.fn(() => {
    sdkStarted.resolve();
    return mode === 'result-ready' ? Promise.resolve(chatTextResponse(PROOF)) : provider.promise;
  });
  const model = new OpenAIChatModel({
    model: 'offline-worker-f19',
  ...
  return {
    ...
    crashTarget: () => {
      if (crashChannelClosed) throw new Error('The F19 crash channel is already closed.');
      crashChannel.postMessage('crash-after-controller-barrier');
    },
    closeCrashChannel: () => {
      if (crashChannelClosed) return;
      crashChannelClosed = true;
      crashChannel.close();
    },
    ...
    terminalizeProviderOutcomeUnknown: async () => {
      const recovered = await ledger.recoverInFlight({
        ownerSessionId: request.ownerSessionId,
        taskId: request.taskId,
        providerOperationId: `worker-f19-provider-${request.taskId}`,
      });
...
async function startPlacement(
  ...
  if ('type' in spawned) throw new Error('Expected a raw Worker task handle.');
...
describe('Worker F19 provider recovery windows', () => {
  acceptanceIt(
    'C7-WORKER-20.l3.result-ready-reply-loss',
          ...
          expect(outcome).toMatchObject({
            ...
            result: {
              ...
              output: {
                ...
                providerOperationId: `worker-f19-provider-${harness.request.taskId}`,
              },
            },
          });
          if (handle !== undefined) {
  ...
  acceptanceIt(
    'C7-WORKER-21.l3.provider-in-flight-unknown',
  ...
  acceptanceIt(
    'C7-WORKER-25.l3.internal-recovery-epoch-quota',
    ...
    async () => {
      ...
      try {
        ...
        const resumeBase = createExecutorConformanceRequest({
          ...
          operation: {
            type: 'resume',
            operationId: 'worker-f19-new-core-epoch-resume',
            reason: 'checkpoint',
        ...
        const resume = Object.freeze({
          ...resumeBase,
          attempt: 2,
          executionEpoch: 'worker-f19-new-core-epoch-2',
