...
if (isMainThread) {
  throw new Error('The Worker conformance target must run in a worker thread.');
}
...
const definition = defineSubAgent({
  ...
  version: '2',
  description: 'Run deterministic Worker placement conformance scenarios.',
  inputSchema: z
    .object({
...
const nestedExecutorDescriptor = Object.freeze({
  ...
  name: 'worker',
  description: 'Worker conformance nested placement.',
        ...
        async run(child, control) {
            ...
            return await new Promise((resolve) => {
              if (scenario === 'cancel') {
                child.signal.addEventListener(
                  'abort',
                  async () => {
                    await control.reportProgress('worker-cancel-observed', {
              ...
              // controller must use its bounded Worker.terminate() fallback.
            });
          }
          ...
          if (scenario === 'approval-resume' && child.checkpoint === undefined) {
            ...
            const directive = await control.authorizeTool(
              'worker-conformance-approval',
              {
                callId: 'conformance-sensitive-call',
                toolName: 'conformance-sensitive-tool',
                summary: 'Approve the deterministic Worker conformance action.',
              },
              checkpoint,
            );
            if (directive.type !== 'suspend') {
              throw new Error('The first Worker approval conformance run must suspend.');
            }
            return {
              type: 'paused',
              reason: 'approval',
          ...
          if (scenario === 'checkpoint-crash' && child.checkpoint === undefined) {
            await control.commitCheckpoint(
              'worker-checkpoint-before-crash',
          ...
          if (
            ...
            await control.reportProgress('worker-resume-scope-evidence', {
          ...
          if (scenario === 'nested-spawn-wait') {
            ...
            await control.completion.submitResult('worker-result:nested-spawn-wait', output);
            await control.completion.complete('worker-end:nested-spawn-wait', {
              isStandalone: true,
            });
            return {
              type: 'terminal',
            ...
            scenario === 'environment'
              ...
              : scenario === 'isolation'
                ? {
                    ...
                    taskId: child.taskId,
                    workerJobId: readClosedWorkerData().jobId ?? null,
                  }
                : { factoryCreates };
          const output = {
            answer: `worker:${scenario}`,
            details: details ?? {},
          };
          await control.completion.submitResult(`worker-result:${scenario}`, output);
          await control.completion.complete(`worker-end:${scenario}`, { isStandalone: true });
          return {
            type: 'terminal',
            result: {
              status: 'succeeded',
...
function createApprovalCheckpoint() {
  ...
  return {
    ...
    pendingBatch: {
      version: '1',
      batchId: 'worker-conformance-approval-batch',
      assistantMessage: { protocol: 'openai-chat', codecVersion: '1', value: [] },
      calls: [
        {
          version: '1',
          operationId: 'worker-conformance-sensitive-operation',
          kind: 'tool',
          callId: 'conformance-sensitive-call',
...
function readEnvironmentEvidence() {
  const closedWorkerData = readClosedWorkerData();
  return {
