...
| `SubAgentTransportExecutorBridge` / `SubAgentTransportTargetBridge`                 | placement-neutral 调度桥、owner/channel 分区、receipt 与可选同进程 rebind。 |
...
`SubAgentTargetRunnerRegistry` 在 seal 后发布不可变的 definition/runner/checkpoint compatibility snapshot，并在 factory 调用前重新校验完整 execution request、schema、binding 与 checkpoint。transport-ready registration 必须声明 `{ gatewayId, protocol, codecVersion }`；该身份会写入持久 `SubAgentExecutorBinding`，resume 时必须精确一致，因此 target 重启后不能在不改变 binding 的情况下静默切换 Model gateway。Gateway ID 应由宿主按实际 provider/model 配置做版本化。普通 `prepareExecution()` 仍拒绝 reconnect；新增的 `prepareExternalReconnect()` 只是供已先证明 exact resident binding 与 controller-ACK checkpoint 的 trusted placement bridge 使用的 compatibility-only preparation SPI，不是独立的恢复授权入口。`SubAgentTransportExecutorBridge` 把可信 Core control 和 controller Model 留在宿主侧；`SubAgentTransportTargetBridge` 只在 target 侧重建受信任 runner。Target bridge 的 `ownerSessionId` 与每次 handler request 的 `channelId` 都来自 placement 已认证的本地 channel 事实，而不是 RPC payload；task、operation、cancel、snapshot、events 与异步 reply 都按 owner + task + channel/generation 分区，跨 session 或过期 attachment 统一返回 `RESOURCE_NOT_FOUND`。placement adapter 可通过异步 `validateBinding({ request, binding })` 在 runner factory 或 resident runner 被使用前校验 opaque recovery data 与受信 bootstrap 身份，例如要求 Worker binding 的 logical job ID 与本次 MessagePort bootstrap 完全一致；该 hook 不替代 Core 对 closed binding、definition、runner、Model binding 和 checkpoint 的通用校验。同一 task 的初始化有独立 single-flight fence，即使 operation ID 不同也不会重复调用 runner factory。通用 bridge 默认最多各保留 10,000 个 task 与 operation receipt，容量耗尽时稳定 fail closed，不会淘汰幂等证据后重跑；`dispose()` 会取消并等待 live runner、清理 listener/timer/receipt，且之后不可复用。

默认配置只允许 settled resident 的只读 reconnect，live external reconnect 仍返回 `RECOVERY_UNSUPPORTED`。placement 只有显式设置 `reconnect: 'external_binding'`、把 `peer` 配置为按 trusted `channelId` 精确解析 Peer 的 channel-aware callback，并提供 durable、幂等且按 execution fencing 校验的 `checkpointCommitted(context)` 时，才启用同一 `SubAgentTransportTargetBridge` 实例内的 fresh-runner recovery；该模式传入单一 scalar Peer 会在构造阶段 fail fast。checkpoint-bearing reverse control 会从 Peer 同步 admission 起，到 controller reply、placement commit 和本地更新全链路串行；reply-loss、timeout、协议错或 commit 失败会留下 gap，并在旧 runner 被终止前阻止恢复，admission 前即可证明的关闭通道则不会污染已有 durable checkpoint。合法 live reconnect 必须来自不同的 fresh trusted `channelId`：bridge 先停止新 reverse activity、排空已准入的 mutating control/checkpoint 与 provider Model exchange，再 fence 旧 channel、等待旧 runner quiesce，并从最后一个 controller-ACK 且 placement 已提交的兼容 checkpoint 重建原 task；已经准入的 events 只允许按原 attachment 完成，hard fence 后的迟到结果会被 quarantine，不能污染新 runner。每个 `providerOperationId` 在 target 内 single-flight；Model packet 同步 admission 后没有完整 correlated reply 时，bridge 保存重新计算的 canonical request hash，只有同一 ID、同一真实语义的 durable-result lookup 可以消除纯 transport 不确定，声明 hash 与 body 不一致或不同语义都在 Peer 前失败。controller 已返回 `outcomeUnknown`、runner 在 Model 结果未决时 settle，或 lookup 仍无法证明原结果时，unknown 单调且不可逆，原 task 禁止 resume/reconnect，也绝不授权新的 provider dispatch。factory 或 deadline 失败后保留 detached recovery slot，后续更高 execution scope 还必须换用另一条 fresh channel 才可重试，旧 channel 的 cancel/snapshot/events/settlement 不能触碰新 runner。该能力不持久化 bridge task map，不导入旧 Peer sequence/pending exchange，也不能在 target bridge 或进程退出后自行恢复；HTTP durable job、spool、attachment generation 与跨进程 restore 仍须由后续 placement 实现。
...
HTTP companion 包当前只负责把一个 Core Peer packet 变成严格、单次签名的 multipart body，并提供 HMAC-SHA256 v1、分布式 replay cache SPI、owner scope authorization 顺序与固定安全 admission 结果。它不创建 listener、job 或 `SubAgentExecutor`，也不实现 heartbeat handler、event cursor、resume/reconnect；这些仍属于后续 HTTP placement。完整 wire、安全边界与 API 见 [HTTP wire-security README](../executor-http/README.md)。
