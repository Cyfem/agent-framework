...
const TARGET_ENTRY = new URL('./fixtures/process-protocol-target.mjs', import.meta.url);
const PROOF = 'process-protocol-proxy-proof';
...
function createExpectedManifest(): SubAgentTargetRunnerManifest {
  ...
  return new SubAgentTargetRunnerRegistry()
    .register({
      ...
      runnerId: 'process-chat-child-runner',
      runnerVersion: '2.0.0',
    ...
    .register({
      ...
      runnerId: 'process-responses-child-runner',
      runnerVersion: '2.0.0',
      childCheckpointVersions: ['1'],
      modelBinding: {
...
function createManifestDefinition(name: string): SubAgentDefinitionRegistration {
  return Object.freeze({
    name,
    version: '2',
    description: 'Manifest-only erased Process protocol definition.',
    inputSchema: z.json(),
    outputSchema: z.json(),
  });
}
...
function createChatHarness(): ProtocolHarness {
  const sdkCreate = vi
    ...
    .mockResolvedValueOnce(
      chatToolResponse('process-chat-result', 'agent-result', {
        result: { proof: PROOF },
      }),
    )
    .mockResolvedValueOnce(chatToolResponse('process-chat-end', 'end-agent', {}));
  const model = new OpenAIChatModel({
    model: 'offline-process-chat',
...
function createResponsesHarness(): ProtocolHarness {
  const sdkCreate = vi
    .fn()
    .mockResolvedValueOnce(
      responsesToolResponse('process-responses-result', 'agent-result', {
        result: { proof: PROOF },
      }),
    )
    .mockResolvedValueOnce(responsesToolResponse('process-responses-end', 'end-agent', {}));
  const model = new OpenAIResponsesModel({
    model: 'offline-process-responses',
...
function createHarness(options: {
  ...
  const taskId = `process-protocol-${options.definitionName}`;
  ...
  const gateway = new SubAgentTransportModelGatewayHandler({
    ...
    acknowledgeCheckpoint: (operation) => {
      ...
      if (stored === undefined) {
        throw new Error('The Process target requested a Model before its checkpoint ACK.');
      }
...
async function executeProtocolHarness(harness: ProtocolHarness, definitionName: string) {
  ...
  const taskId = `process-protocol-${definitionName}`;
  const request = createExecutorConformanceRequest({
    executorName: 'process',
    taskId,
...
describe('Process placement Chat and Responses Model proxies', () => {
  acceptanceIt('C7-PROCESS-08.l4.chat-model-proxy', 'fake-openai-chat-sdk', async () => {
    await executeProtocolHarness(createChatHarness(), chatDefinition.name);
  });

  acceptanceIt('C7-PROCESS-09.l4.responses-model-proxy', 'fake-openai-responses-sdk', async () => {
