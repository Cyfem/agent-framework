...
function validBinding(jobId = 'process-job-1') {
  return { kind: PROCESS_SUBAGENT_BINDING_KIND, jobId };
}

function expectInvalid(value: unknown, label = 'invalid Process binding'): void {
  expect(() => decodeProcessBinding(value), label).toThrow(
    expect.objectContaining({ code: 'BINDING_INVALID' }),
  );
}

describe('Process binding codec closed boundary', () => {
  acceptanceIt('C7-PROCESS-10.l1.binding-closed-shape', 'hostile-object-boundary', () => {
    expect(decodeProcessBinding(validBinding('a'))).toEqual(validBinding('a'));
    expect(decodeProcessBinding(validBinding(`a${'z'.repeat(127)}`))).toEqual(
      validBinding(`a${'z'.repeat(127)}`),
    );
    expect(processSubAgentBindingCodec.encode(validBinding())).toEqual(validBinding());
    ...
    expectInvalid([]);
    expectInvalid({});
    expectInvalid({ jobId: 'process-job-1' });
    expectInvalid({ kind: PROCESS_SUBAGENT_BINDING_KIND });
    expectInvalid({ ...validBinding(), extra: true });
    expectInvalid({ ...validBinding(), kind: 'maneeagent-process/v2' });
  ...
  it('rejects Proxy, symbols, non-enumerable fields, accessors and custom prototypes', () => {
    ...
    const accessor = Object.defineProperties(
      {},
      {
        kind: {
          configurable: true,
          enumerable: true,
          value: PROCESS_SUBAGENT_BINDING_KIND,
        },
        jobId: {
          configurable: true,
          enumerable: true,
    ...
    class HostileBinding {
      readonly kind = PROCESS_SUBAGENT_BINDING_KIND;
      readonly jobId = 'process-job-1';
    }
    expectInvalid(new HostileBinding(), 'custom prototype');
    expect(
      decodeProcessBinding(Object.assign(Object.create(null) as object, validBinding())),
