...
import { decodeProcessBinding, ProcessSubAgentExecutor } from '../src';
import { assertNetworkDenyGuardInstalled } from './network-deny.setup';

const TARGET_ENTRY = new URL('./fixtures/process-completion-crash-target.mjs', import.meta.url);
const DEFINITION_REF = Object.freeze({ name: 'process-completion-crash-child', version: '2' });
const RESULT_CALL_ID = 'process-completion-crash-result';
const COMPLETION_CALL_ID = 'process-completion-crash-end';
const PROOF = 'process-authoritative-completion-proof';
...
const definition: SubAgentDefinitionRegistration = Object.freeze({
  ...DEFINITION_REF,
  description: 'Exercise Process completion control reply-loss windows.',
  inputSchema: z
    .object({
...
async function createHarness(scenario: CrashScenario): Promise<CrashHarness> {
  ...
  const sessionId = `process-completion-session-${suffix}`;
  const runId = `process-completion-run-${suffix}`;
  const barrierId = `process-completion-${suffix}`;
  const store = new CompletionCrashStateStore(
    scenario === 'result-receipt' ? 'result_submitted' : 'succeeded',
    `process-completion-store-${suffix}`,
  );
  ...
  return {
    ...
    runtime,
    model,
    crashPath: join(tmpdir(), `maneeagent-process-completion-crash-${barrierId}.signal`),
  };
}
...
async function spawnCrashTask(
  ...
  return harness.runtime.spawn({
    runId: harness.runId,
    requestId: `process-completion-request-${scenario}`,
...
function expectedOutput(task: StoredTask, scenario: CrashScenario) {
  if (task.executionEpoch === undefined || task.executionFencingToken === undefined) {
    throw new Error('The authoritative Process execution scope is missing.');
  }
  return {
    proof: PROOF,
    scenario,
...
function crashAfterBarrier(harness: CrashHarness): void {
  writeFileSync(harness.crashPath, 'crash-after-authoritative-cas', { flag: 'wx' });
}
...
async function expectTerminalReadOnly(
  ...
  await expect(
    harness.runtime.cancel(harness.sessionId, handle.taskId, 'terminal-read-only', {
      operationId: 'process-terminal-read-only-cancel',
    }),
...
async function expectOneLogicalJob(
  ...
  if (task.binding === undefined) throw new Error('The Process task binding is missing.');
  if (originalBinding === undefined) throw new Error('The original Process binding is missing.');
  const decoded = decodeProcessBinding(task.binding.recoveryData);
  expect(decoded.jobId).toBe(decodeProcessBinding(originalBinding.recoveryData).jobId);
  return task;
}
...
describe('Process authoritative completion crash windows', () => {
  acceptanceIt(
    'C7-PROCESS-28.l3.result-receipt-cas-reply-loss',
    ...
    async () => {
      ...
      try {
        ...
        harness.store.release();
        rmSync(harness.crashPath, { force: true });
  ...
  acceptanceIt(
    'C7-PROCESS-29.l3.terminal-cas-completion-reply-loss',
    ...
    async () => {
      ...
      try {
        ...
        if (terminal.type !== 'terminal') {
          throw new Error('The F09 Process task did not preserve its terminal outcome.');
        }
