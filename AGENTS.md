...
- 根据改动范围继续阅读相关项目文档：
  ...
  - 当前 checkout 已完成 C6，并交付 C7a/C7b/C7c-1 的 Core transport/RPC/control/Peer/artifact-sidecar、recoverable Executor settle、target registry、controller/target bridge 与 controller-owned Model gateway，以及 C7c-2/C7c-3 的离线 Worker/Process placement；HTTP placement 尚未交付、Phase 2 未通过，C8–C9 分布式能力仍以源码、测试和 README 状态为准。
  - Core/Local/Worker/Process package manifest 已固定为待发布 `2.0.0`，但 npm Core `latest` 仍是 `1.0.0`、三个 Executor 包尚未发布；不得把 manifest、pack 成功或计划状态写成 npm 已发布。
